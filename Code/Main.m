...
K = 10; % Tested with 6, 8, and 10
ihts_method = "ihts_smote"; % Alternatives: "ihts", "ihts_simple", and "ihts_smote"
save_results = false;
% Cutoff used to find ROC curve. If ROC not relevant, set as [0.5 0.5]
%cutoffs = [0.1 0.9; 0.2 0.8; 0.3 0.7; 0.4 0.6; 0.5 0.5; 0.6 0.4; 0.7 0.3; 0.8 0.2; 0.9 0.1];
cutoffs = [0.5 0.5];
...
for k=1:size(cutoffs, 1)
    ...
    for i = 1:length(partitions)
        ...
        model = classRF_train(observations, classes, 500, floor(sqrt(size(observations,2))), extra_options);
    ...
    sp_sn = [1-sp_average sn_average];
    roc_values = [roc_values; sp_sn];
    
    results = [sn_average sp_average acc_average mcc_average]
end

roc_values = [roc_values; 1 1];

auc = trapz(roc_values(:,1), roc_values(:,2))
